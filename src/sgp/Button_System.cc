...
GUIButtonRef CreateTextButton(const ST::string& str, SGPFont font, INT16 sForeColor, INT16 sShadowColor, INT16 xloc, INT16 yloc, INT16 w, INT16 h, INT16 Priority, GUI_CALLBACK ClickCallback)
{
	...
	if (sForeColor == FONT_MCOLOR_RED && sShadowColor == FONT_BLACK)
	{
		b->SpecifyOS0Style();
	}
	return b;
...
static void DrawGenericButton(const GUI_BUTTON* b)
{
	if (b->fOS0Style)
	{
		const BOOLEAN active = b->Clicked() || (b->Area.uiFlags & MSYS_MOUSE_IN_AREA);
		const UINT16 fill = Get16BPPColor(active ? FROMRGB(28, 8, 11) : FROMRGB(5, 7, 8));
		const UINT16 edge = Get16BPPColor(active ? FROMRGB(220, 24, 32) : FROMRGB(112, 10, 16));
		ColorFillVideoSurfaceArea(ButtonDestBuffer, b->X(), b->Y(), b->BottomRightX(), b->BottomRightY(), fill);
		ColorFillVideoSurfaceArea(ButtonDestBuffer, b->X(), b->Y(), b->BottomRightX(), b->Y() + 1, edge);
		ColorFillVideoSurfaceArea(ButtonDestBuffer, b->X(), b->BottomRightY() - 1, b->BottomRightX(), b->BottomRightY(), edge);
		ColorFillVideoSurfaceArea(ButtonDestBuffer, b->X(), b->Y(), b->X() + 1, b->BottomRightY(), edge);
		ColorFillVideoSurfaceArea(ButtonDestBuffer, b->BottomRightX() - 1, b->Y(), b->BottomRightX(), b->BottomRightY(), edge);
		return;
	}
