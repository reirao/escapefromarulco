...
void InitNewGame()
{
	...
	{ // Our first time, go into laptop
		...
		// Escape from Arulco sandbox: no campaign briefing or IMP marketing mail.
		...
		// Escape from Arulco: enter the owned, enemy-free start sector with a
		// temporary operator.  Character/perk editing now belongs to the live
		// tactical simulation instead of a boot-time computer workflow.
		StartOS0SandboxWithDefaultOperator();
		gubScreenCount = 1;
