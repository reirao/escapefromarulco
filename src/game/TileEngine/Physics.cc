...
WORLD_PHYSICS_PROFILE GetWorldPhysicsProfile(STRUCTURE const* const structure)
{
	WORLD_PHYSICS_PROFILE profile{
		"UNKNOWN", 1000.0f, 0.8f, 0.02f, 0, FALSE
	};
	if (!structure || !structure->pDBStructureRef ||
		!structure->pDBStructureRef->pDBStructure) return profile;

	DB_STRUCTURE const* const db = structure->pDBStructureRef->pDBStructure;
	float materialMass = 32.0f;
	switch (db->ubArmour)
	{
		case MATERIAL_WOOD_WALL:
		case MATERIAL_PLYWOOD_WALL:
		case MATERIAL_LIVE_WOOD:
			profile.materialName = "WOOD";
			materialMass = 24.0f;
			profile.friction = 0.62f;
			profile.restitution = 0.12f;
			break;
		case MATERIAL_LIGHT_VEGETATION:
		case MATERIAL_CACTUS:
			profile.materialName = "ORGANIC";
			materialMass = 9.0f;
			profile.friction = 0.72f;
			profile.restitution = 0.08f;
			break;
		case MATERIAL_FURNITURE:
			profile.materialName = "FURNITURE";
			materialMass = 28.0f;
			profile.friction = 0.58f;
			profile.restitution = 0.10f;
			break;
		case MATERIAL_PORCELAIN:
			profile.materialName = "CERAMIC";
			materialMass = 18.0f;
			profile.friction = 0.48f;
			profile.restitution = 0.04f;
			break;
		case MATERIAL_STONE:
		case MATERIAL_CONCRETE1:
		case MATERIAL_CONCRETE2:
		case MATERIAL_ROCK:
		case MATERIAL_INDESTRUCTABLE_STONE:
			profile.materialName = "STONE";
			materialMass = 92.0f;
			profile.friction = 0.82f;
			profile.restitution = 0.02f;
			break;
		case MATERIAL_RUBBER:
			profile.materialName = "RUBBER";
			materialMass = 16.0f;
			profile.friction = 0.94f;
			profile.restitution = 0.68f;
			break;
		case MATERIAL_SAND:
		case MATERIAL_SANDBAG:
			profile.materialName = "SAND";
			materialMass = 38.0f;
			profile.friction = 0.91f;
			profile.restitution = 0.01f;
			break;
		case MATERIAL_CLOTH:
			profile.materialName = "CLOTH";
			materialMass = 6.0f;
			profile.friction = 0.70f;
			profile.restitution = 0.03f;
			break;
		case MATERIAL_LIGHT_METAL:
			profile.materialName = "LIGHT METAL";
			materialMass = 42.0f;
			profile.friction = 0.44f;
			profile.restitution = 0.20f;
			break;
		case MATERIAL_THICKER_METAL:
		case MATERIAL_HEAVY_METAL:
		case MATERIAL_INDESTRUCTABLE_METAL:
		case MATERIAL_THICKER_METAL_WITH_SCREEN_WINDOWS:
			profile.materialName = "HEAVY METAL";
			materialMass = 105.0f;
			profile.friction = 0.50f;
			profile.restitution = 0.14f;
			break;
		default:
			profile.materialName = "COMPOSITE";
			break;
	}

	const float density = std::clamp(db->ubDensity / 100.0f, 0.25f, 1.0f);
	profile.massKg = materialMass * density *
		std::max<UINT8>(1, db->ubNumberOfTiles);
	profile.integrity = structure->ubHitPoints;
	...
	profile.portableObject = db->ubNumberOfTiles == 1 &&
		structure->fFlags & STRUCTURE_BASE_TILE && !(structure->fFlags & fixed);
	return profile;
}
