...
void LoadAllMapChangesFromMapTempFileAndApplyThem()
{
	...
	for( cnt=0; cnt< uiNumberOfElements; cnt++ )
	{
		...
		switch( pMap->ubType )
		{
			...
			case SLM_LAND:
			{
				UINT16 const index = GetTileIndexFromTypeSubIndex(
					pMap->usImageType, pMap->usSubImageIndex);
				BOOLEAN exists = FALSE;
				for (LEVELNODE const* land = gpWorldLevelData[pMap->usGridNo].pLandHead;
					land; land = land->pNext)
				{
					if (land->usIndex == index) { exists = TRUE; break; }
				}
				if (!exists) AddLandToHead(pMap->usGridNo, index);
			...
			case SLM_REMOVE_LAND:
			{
				UINT16 const index = GetTileIndexFromTypeSubIndex(
					pMap->usImageType, pMap->usSubImageIndex);
				RemoveLand(pMap->usGridNo, index);
				SaveModifiedMapStructToMapTempFile(pMap, *tempMapFile);
				uiNumberOfElementsSavedBackToFile++;
				break;
			}
			case SLM_REMOVE_OBJECT:
			{
				UINT16 const index = GetTileIndexFromTypeSubIndex(
					pMap->usImageType, pMap->usSubImageIndex);
				RemoveObject(pMap->usGridNo, index);
				SaveModifiedMapStructToMapTempFile(pMap, *tempMapFile);
				uiNumberOfElementsSavedBackToFile++;
				break;
			}
...
void AddLandToMapTempFile(UINT32 const mapIndex, UINT16 const tileIndex)
{
	AddToMapTempFile(mapIndex, tileIndex, SLM_LAND);
}


void RemoveLandFromMapTempFile(UINT32 const mapIndex, UINT16 const tileIndex)
{
	AddToMapTempFile(mapIndex, tileIndex, SLM_REMOVE_LAND);
}
