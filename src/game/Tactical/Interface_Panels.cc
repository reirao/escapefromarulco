...
void HideLegacyTacticalInterfaceForOS0()
{
	if (guiCurrentScreen != GAME_SCREEN) return;

	// The old panel can be enabled again by squad updates, returning from the
	// map, dialogue cleanup and modal tactical states. Make OS//0 ownership an
	// invariant instead of a one-time initialization side effect.
	gRadarRegion.Disable();
	if (gsCurInterfacePanel == TEAM_PANEL)
	{
		gTEAM_PanelRegion.Disable();
		for (GUIButtonRef& button : iTEAMPanelButtons)
		{
			if (button) HideButton(button);
		}
		for (TeamPanelSlot& slot : gTeamPanel)
		{
			slot.face.Disable();
			slot.enemy_indicator.Disable();
			slot.bars.Disable();
			slot.left_bars.Disable();
			slot.first_hand.Disable();
			slot.second_hand.Disable();
		}
	}
	else if (gsCurInterfacePanel == SM_PANEL)
	{
		gSMPanelRegion.Disable();
		gSM_SELMERCPanelRegion.Disable();
		gSM_SELMERCBarsRegion.Disable();
		gSM_SELMERCMoneyRegion.Disable();
		gSM_SELMERCEnemyIndicatorRegion.Disable();
		for (MOUSE_REGION& region : gSM_SELMERCStatsRegion) region.Disable();
		DisableInvRegions(TRUE);
		for (GUIButtonRef& button : iSMPanelButtons)
		{
			if (button) HideButton(button);
		}
		if (giSMStealthButton) HideButton(giSMStealthButton);
	}
}
...
void InitializeSMPanel()
{
	...
	MSYS_DefineRegion(&gViewportRegion, 0, 0, gsVIEWPORT_END_X, gsVIEWPORT_WINDOW_END_Y, MSYS_PRIORITY_NORMAL, VIDEO_DEFAULT_TO_NO_CURSOR, TacticalViewPortMovementCallback, TacticalViewPortTouchCallback);
...
void InitializeTEAMPanel()
{
	...
	// Set global mouse regions
	// Define region for viewport
	MSYS_DefineRegion(&gViewportRegion, 0, 0, gsVIEWPORT_END_X, gsVIEWPORT_END_Y, MSYS_PRIORITY_NORMAL, VIDEO_DEFAULT_TO_NO_CURSOR, TacticalViewPortMovementCallback, TacticalViewPortTouchCallback);
	...
	// The fixed JA2 team bar is retained internally for compatibility, but its
	// controls must not form an invisible click layer over OS//0's full-height
	// tactical world.
	if (guiCurrentScreen == GAME_SCREEN)
	{
		gTEAM_PanelRegion.Disable();
		...
		for (GUIButtonRef& button : iTEAMPanelButtons) HideButton(button);
	}
}
