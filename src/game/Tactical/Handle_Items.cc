...
void SoldierPickupItem( SOLDIERTYPE *pSoldier, INT32 iItemIndex, INT16 sGridNo, INT8 bZLevel )
{
	...
	if ( pSoldier->sGridNo != sActionGridNo )
	{
		...
		{
			if (!EVENT_InternalGetNewSoldierPath(pSoldier, sActionGridNo,
...
void HandleSoldierPickupItem(SOLDIERTYPE* const s, INT32 const item_idx, INT16 const gridno, INT8 const z_level)
{
	...
	// Escape from Arulco never creates the legacy modal pickup list. OS//0 can
	// pass an exact item index; after JA2 has walked the operator into range and
	// played the pickup animation, complete that native pickup normally.
	if (s->bTeam == OUR_TEAM)
	{
		if (item_idx >= 0 || item_idx == ITEM_PICKUP_ACTION_ALL)
		{
...
BOOLEAN SetItemsVisibilityOn(GridNo const grid_no, UINT8 const level, Visibility const bAllGreaterThan, BOOLEAN const fSetLocator)
{
	...
	// Escape from Arulco presents discovered objects in the spatial loot
	// container. Do not flash/radio-pulse them in the world.
	(void)fSetLocator;
	return TRUE;
}
...
{
	// Object interaction is visual and drag-and-drop in OS//0. The original
	// hover list duplicates that interface and is intentionally suppressed.
	(void)pItemPool;
	(void)bZLevel;
	(void)sXPos;
	(void)sYPos;
