	...
	switch( gsCurInterfacePanel )
	{
		case SM_PANEL:
			...
			gsVIEWPORT_WINDOW_END_Y =
				guiCurrentScreen == GAME_SCREEN ? SCREEN_HEIGHT : INV_INTERFACE_START_Y;
		...
		case TEAM_PANEL:
			gsVIEWPORT_WINDOW_END_Y =
				guiCurrentScreen == GAME_SCREEN ? SCREEN_HEIGHT : INTERFACE_START_Y;
			// Render full
			SetRenderFlags(RENDER_FLAG_FULL);
			InitializeTEAMPanel();
			break;
...
{
	// OS//0 owns these actions in tactical play.  Recreating the native panel
	// buttons after a dialogue or mode change used to make the old inventory,
	// map, end-turn and squad controls leak back into the bottom of the screen.
