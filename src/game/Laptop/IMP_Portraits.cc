...
void RenderIMPPortraits( void )
{
	...
	// Draw the OS//0 information panel before the portrait.  The flat panel is
	// opaque, so drawing it last used to hide the face completely.
...
static void MakeButton(UINT idx, const char* img_file, INT32 off_normal, INT32 on_normal, const ST::string& text, INT16 x, INT16 y, GUI_CALLBACK click)
{
	...
	GUIButtonRef const btn = CreateTextButton(text, FONT12ARIAL, FONT_MCOLOR_RED, FONT_BLACK,
		x, y, 92, 42, MSYS_PRIORITY_HIGH, click);
