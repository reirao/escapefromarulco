...
static BOOLEAN IsOS0OperatorMode()
{
	return guiCurrentLaptopMode == LAPTOP_MODE_CHAR_PROFILE ||
...
void InitLaptopAndLaptopScreens(void)
{
	...
	// The tactical bootstrap still uses FinalizeIMPCharacter once to instantiate
	// the temporary operator. It sets this flag after that succeeds. Setting it
	// here would skip soldier creation entirely and leave the boot screen black.
...
static void EnterLaptop(void)
{
	...
	// Character creation belongs to the tactical OS//0 introduction. The
	// laptop opens as a neutral desktop and never boots the retired IMP pages.
	guiCurrentWWWMode     = LAPTOP_MODE_NONE;
	guiCurrentLaptopMode  = LAPTOP_MODE_NONE;
	guiPreviousLaptopMode = LAPTOP_MODE_NONE;
	...
	// Remove an IMP bookmark inherited from an older save while preserving the
	// order of every still-valid laptop bookmark.
	auto const bookmarksEnd = std::remove(
		std::begin(LaptopSaveInfo.iBookMarkList),
		std::end(LaptopSaveInfo.iBookMarkList),
		IMP_BOOKMARK);
	std::fill(bookmarksEnd, std::end(LaptopSaveInfo.iBookMarkList), -1);
...
static void RenderLapTopImage(void)
{
	...
	if (IsOS0OperatorMode())
	{
		// Escape from Arulco: the operator creator is a local OS//0 tool, not
		// a Sir-Tech laptop application.  Replace the complete laptop shell.
		const UINT16 black = Get16BPPColor(FROMRGB(3, 4, 5));
		const UINT16 graphite = Get16BPPColor(FROMRGB(18, 20, 22));
		...
		for (GUIButtonRef const& button : gLaptopButton) HideButton(button);
		MarkButtonsDirty();
		return;
	}

	for (GUIButtonRef const& button : gLaptopButton) ShowButton(button);
...
static void RenderLaptop(void)
{
	...
	if (guiCurrentLaptopMode >= LAPTOP_MODE_WWW && !IsOS0OperatorMode())
	{
...
do_nothing:
	...
	if (fFirstTimeInLaptop && guiCurrentLaptopMode >= LAPTOP_MODE_WWW && !IsOS0OperatorMode())
	{
...
ScreenID LaptopScreenHandle()
{
	...
	// OS//0 operator creation has no mail, web bookmarks or laptop taskbar.
	if (!IsOS0OperatorMode())
	{
		CreateDestroyNewMailButton();
		DisplayNewMailBox();
	...
	if (!IsOS0OperatorMode()) DisplayWebBookMarkNotify();
...
{
	// Old mail scripts may still request this bookmark. The tactical operator
	// creator replaces IMP completely, so silently discard that request.
	if (iBookId == IMP_BOOKMARK) return;
...
void GoToWebPage(INT32 iPageId)
{
	...
	switch (iPageId)
	{
		...
		case IMP_BOOKMARK:
			// Stale bookmark region from an old save: return to the desktop.
			guiCurrentWWWMode     = LAPTOP_MODE_NONE;
			guiCurrentLaptopMode  = LAPTOP_MODE_NONE;
			guiPreviousLaptopMode = LAPTOP_MODE_NONE;
			break;
...
static void DisplayLoadPending(void)
{
	...
	// The operator core is local.  Complete its state transition immediately
	// instead of waiting for the vanilla simulated website download.
	if (IsOS0OperatorMode())
	{
		fLoadPendingFlag        = FALSE;
		fFastLoadFlag           = FALSE;
...
static void DrawDeskTopBackground(void)
{
	if (IsOS0OperatorMode())
	{
		...
		SetFontForeground(FONT_MCOLOR_DKGRAY);
		MPrint(STD_SCREEN_X + 78, STD_SCREEN_Y + 7, "OPERATOR CREATION  /  LOCAL SESSION");
		MPrint(STD_SCREEN_X + 244, STD_SCREEN_Y + 232, "INITIALIZING OPERATOR CORE...");
		return;
	}
