...
static GUIButtonRef MakeButton(BUTTON_PICS* const img, const ST::string& text, const INT16 x, const INT16 y, const INT16 width, GUI_CALLBACK click)
{
	GUIButtonRef const btn = CreateTextButton(text, FONT12ARIAL, FONT_MCOLOR_RED, FONT_BLACK,
		x, y, width, 42, MSYS_PRIORITY_HIGH, std::move(click));
...
static void CreateIMPPersonalityQuizButtons(void)
{
	...
	giIMPPersonalityQuizButton[0] = MakeButton(giIMPPersonalityQuizButtonImage[0], pImpButtonText[8], dx + 290, dy + 302, 180, BtnIMPPersonalityQuizAnswerConfirmCallback);

	// start over
	giIMPPersonalityQuizButtonImage[1] = LoadButtonImage(LAPTOPDIR "/button_5.sti", 0, 1);
	giIMPPersonalityQuizButton[1] = MakeButton(giIMPPersonalityQuizButtonImage[1], pImpButtonText[7], dx + 30, dy + 302, 180, BtnIMPPersonalityQuizStartOverCallback);

	giPreviousQuestionButtonImage = LoadButtonImage(LAPTOPDIR "/button_3.sti", 0, 1);
	giPreviousQuestionButton = MakeButton(giPreviousQuestionButtonImage, pImpButtonText[12], dx + 100, dy + 361, 135, PreviousQuestionButtonCallback);

	giNextQuestionButtonImage = LoadButtonImage(LAPTOPDIR "/button_3.sti", 0, 1);
	giNextQuestionButton = MakeButton(giNextQuestionButtonImage, pImpButtonText[13], dx + 265, dy + 361, 135, NextQuestionButtonCallback);
...
static void AddIMPPersonalityQuizAnswerButtons(INT32 iNumberOfButtons)
{
	...
	for (INT32 i = 0; i < iNumberOfButtons; i++)
	{
		...
		GUIButtonRef const Button = CreateTextButton(sString, FONT12ARIAL,
			FONT_MCOLOR_RED, FONT_BLACK, XLoc, YLoc, 40, 34,
			MSYS_PRIORITY_HIGHEST - 3, BtnQuizAnswerCallback);
		giIMPPersonalityQuizAnswerButton[i] = Button;
		Button->SetUserData(i);
		Button->SpecifyHilitedTextColors(FONT_WHITE, FONT_BLACK);
