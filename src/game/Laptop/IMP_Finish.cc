...
// buttons
static BUTTON_PICS* giIMPFinishButtonImage[7];
GUIButtonRef giIMPFinishButton[7];
...
static void MakeButton(UINT idx, const char* img_file, const ST::string& text, INT16 x, INT16 y, GUI_CALLBACK click)
{
	...
	GUIButtonRef const btn = CreateTextButton(text, FONT12ARIAL, FONT_MCOLOR_RED, FONT_BLACK,
		x, y, 105, 44, MSYS_PRIORITY_HIGH, click);
...
static void BtnIMPFinishLoadoutCallback(GUI_BUTTON* btn, UINT32 reason);
...
static void CreateIMPFinishButtons(void)
{
	...
	MakeButton(6, LAPTOPDIR "/button_8.sti", "LOADOUT", dx + 373, dy + 305, BtnIMPFinishLoadoutCallback);
}
...
static void DeleteIMPFinishButtons(void)
{
	...
	RemoveButton(giIMPFinishButton[6] );
	UnloadButtonImage(giIMPFinishButtonImage[6] );
}


static void BtnIMPFinishLoadoutCallback(GUI_BUTTON *, UINT32 reason)
{
