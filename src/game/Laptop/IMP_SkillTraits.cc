...
void EnterIMPSkillTrait( void )
{
	...
	giIMPSkillTraitFinsihButton = CreateTextButton("CONFIRM TRAITS", FONT12ARIAL,
		FONT_MCOLOR_RED, FONT_BLACK, LAPTOP_SCREEN_UL_X + 350,
		LAPTOP_SCREEN_WEB_UL_Y + 340, 125, 42, MSYS_PRIORITY_HIGH,
		(GUI_CALLBACK)BtnIMPSkillTraitFinishCallback);
	giIMPSkillTraitFinsihButton->SpecifyHilitedTextColors(FONT_WHITE, FONT_BLACK);
...
void AddImpSkillTraitButtons()
{
	...
	for(iCnt = 0; iCnt < IMP_SKILL_TRAITS__NUMBER_SKILLS; iCnt++)
	{
		...
		giIMPSkillTraitAnswerButton[iCnt] = CreateTextButton("[ ]", FONT12ARIAL,
									FONT_MCOLOR_RED, FONT_BLACK, usPosX, usPosY, 38, 28,
									MSYS_PRIORITY_HIGHEST - 3,
									(GUI_CALLBACK)BtnIMPSkillTraitAnswerCallback);
		giIMPSkillTraitAnswerButton[iCnt]->SpecifyHilitedTextColors(FONT_WHITE, FONT_BLACK);
		//Set the button data
...
void IMPSkillTraitDisplaySkills()
{
	...
	for( uiCnt=0; uiCnt<IMP_SKILL_TRAITS__NUMBER_SKILLS; uiCnt++ )
	{
		...
		const UINT16 boxColor = Get16BPPColor(gfSkillTraitQuestions[uiCnt]
			? FROMRGB(72, 8, 12) : FROMRGB(12, 14, 16));
		ColorFillVideoSurfaceArea(FRAME_BUFFER, usBoxPosX, usBoxPosY,
			usBoxPosX + 220, usBoxPosY + 27, boxColor);
